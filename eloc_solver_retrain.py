...
class Solver(object):
    def __init__(self, args):
        ...
        if self.args.resume:
            ...
            self.logger.set_names(['Learning Rate', 'Train Loss', 'Valid AUROC'])
    ...
    def val(self, epoch):
        ...
        #acc = (np.sum(cifar >= 0.5) + np.sum(other <= 0.5)) / (len(cifar) + len(other))
    ...
    def save_checkpoint(self, state, is_best, checkpoint='checkpoint', filename='checkpoint.pth.tar'):
        ...
        if is_best:
            shutil.copyfile(filepath, os.path.join(checkpoint, 'retrained_model_best.pth.tar'))
